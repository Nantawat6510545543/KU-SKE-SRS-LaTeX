...
\begin{figure}[h]
    ...
    \caption{Baseline workflow for prestimulus EEG-based behavioral prediction from CCD-ITI segments.}
...
signal inspection → dataset verification → baseline training → evaluation

This ensures that baseline results are directly connected to verified signal characteristics rather than blindly training on raw data.
\pagebreak
...
\begin{itemize}
    \item reduced prediction error (e.g., MAE/MSE) for RT and accuracy
...
\begin{itemize}[leftmargin=3.5em]
    ...
    \item \textbf{Specifications (SPEC):} Evaluate baseline models that map CCD-ITI features to reaction time (RT) and trial-level accuracy as regression targets.
...
\begin{itemize}[leftmargin=3.5em]
    ...
    \item \textbf{Output:} Predicted RT and trial-level accuracy (regression)
    \item \textbf{Success Criteria:} Lower prediction error with stable generalization to unseen subjects.
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption{Training Interface – Dataset selection, baseline configuration, and training log}
...
\begin{figure}[H]
    ...
    \caption{Comparison Interface – Visual comparison between baseline configurations}
...
The AI-based application is deployed as a web-based system in a research environment.
The trained model is hosted on a backend server and accessed through a browser interface.
...
\begin{itemize}
    ...
    \item Model — behavioral prediction baseline
\end{itemize}
...
The baseline model in our system targets prestimulus behavioral prediction from CCD-ITI segments by mapping EEG-derived features to reaction time (RT) and trial-level accuracy as regression targets.
