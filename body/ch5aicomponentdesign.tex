...
\begin{figure}[h]
    \centering
    \begin{tikzpicture}
        \node[anchor=south west, inner sep=0] (img) at (0,0) {\includegraphics[width=0.95\textwidth]{figures/CCD-workflow}};
        \begin{scope}[x={(img.south east)}, y={(img.north west)}]
            \fill[white] (0.275,0.12) rectangle (0.505,0.355);
            \node[align=center, font=\tiny] at (0.39,0.235) {RT Regression +\\Correctness Classification};
        \end{scope}
    \end{tikzpicture}
...
The model pipeline is validated under a subject-independent evaluation setting.
In this setting, the model is trained on a group of subjects and tested on unseen individuals, which better reflects real-world generalization and avoids overly optimistic results from within-subject testing.
