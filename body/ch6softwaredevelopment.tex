...
The core goal was to implement a reproducible pipeline for prestimulus EEG-based behavioral prediction on the HBN-EEG dataset (CCD ITI segments), and to support rapid experimentation with baseline preprocessing and modeling choices.

The development process followed four repeating steps:
\begin{enumerate}[label=\textbf{M\arabic*.}, leftmargin=*]
	\item \textbf{Define a measurable milestone:} e.g., extract CCD ITI prestimulus windows, compute features, train a baseline regressor, or produce evaluation plots.
...
To keep the work aligned with the baseline scope, the model development emphasized:
\begin{itemize}[leftmargin=*]
	\item \textbf{Clear data boundaries} between subjects/sessions when creating splits.
	\item \textbf{Simple baselines first} (feature-based regressors) before attempting more complex architectures.
...
\begin{itemize}[leftmargin=*]
	...
	\item \textbf{Machine learning:} scikit-learn for baseline models and metrics; PyTorch for neural baselines (when applicable).
...
\hline
1: Setup \& scoping & Repository setup, baseline-only scope definition (CCD ITI prestimulus), and report structure alignment & Buildable \LaTeX{} document, chapter plan \\
\hline
...
\hline
4: Baseline modeling & Train baseline regressors for RT and accuracy (as regression targets); compare simple model families and features & Baseline metrics (e.g., MAE/MSE), prediction vs.
ground-truth plots \\
\hline
...
\hline
\end{tabularx}
\caption{Progress tracking summary for the baseline prestimulus behavioral prediction project.}
