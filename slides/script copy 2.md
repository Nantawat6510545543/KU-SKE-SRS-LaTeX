...
Now, BCI research with EEG is not simple.

*(point to graph)* This chart shows the number of subjects available across different HBN dataset releases and experimental tasks.
What it makes clear is that we are dealing with **many experimental tasks on the x-axis** and **a growing, heterogeneous subject pool on the y-axis**.

Each subject has different brain patterns, different noise profiles, and different behavioral responses.
Combine that with dozens of experimental paradigms and you have an analysis space that is **exponentially difficult to manage in a reproducible way**.
Small differences in preprocessing choices can lead to entirely different conclusions.
...
Either use a **commercial tool** — which is easy to get started with but rigid and inflexible —
or write **custom scripts** — which gives full control but demands software engineering *skills* that most domain researchers simply do not have.

The result in practice?
Folders full of script copies: v1, v2, v3 — with unclear differences.
Pipelines that nobody else can run. Experiments that cannot be replicated even by the original author six months later.
...
This architecture is designed as a **layered, modular pipeline** so we can scale experiments without rewriting the system.

From top to bottom:
- The **Presentation Layer** is the React web UI.
- The **API Layer** uses FastAPI and schema validation to ensure consistent requests.
- The **Application Layer** orchestrates tasks like plotting, dataset building, and training.
- The **Domain Processing Layer** handles EEG preprocessing, feature building, and model training.
- The **Infrastructure Layer** persists cache, BIDS data access, and experiment outputs.

The key part is the **extension points** in the middle.
We can plug in a new preprocessor, feature extractor, or model factory without changing the whole pipeline.

So this is not just a diagram of components.
It is the reason our platform is **reproducible, maintainable, and easy to extend** for future EEG tasks.
...
Scoring the project across five dimensions:

Reproducibility and documentation scored **9 out of 10** — the platform is fully configurable and all runs are logged.
Software quality scored **8** and usability **7**.
Predictive performance scored **3** — which is honest and intentional.

The verdict: this is a **strong engineering baseline with limited AI performance**.
The platform is ready. The model is the open technical problem.
